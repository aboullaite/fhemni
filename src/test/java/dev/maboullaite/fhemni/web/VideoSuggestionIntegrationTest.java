...
class VideoSuggestionIntegrationTest {
    ...
    void requiresLoginDeduplicatesSuggestionsAndProtectsTheAdminQueue() throws Exception {
        ...
        mvc.perform(get("/api/auth/session").with(oauth2Login()))
                ...
                .andExpect(jsonPath("$.chatQuota.weeklyLimit").value(100))
                .andExpect(jsonPath("$.chatQuota.used").value(0))
                .andExpect(jsonPath("$.chatQuota.remaining").value(100))
                .andExpect(jsonPath("$.chatQuota.resetsAt").isNotEmpty())
                .andExpect(jsonPath("$.chatQuota.dailyRequestLimit").value(20))
                .andExpect(jsonPath("$.chatQuota.dailyRequestsUsed").value(0))
                .andExpect(jsonPath("$.chatQuota.dailyRequestsRemaining").value(20))
                .andExpect(jsonPath("$.chatQuota.dailyRequestsResetAt").isNotEmpty())
