(function () {
    ...
    const COPY = {
        ar: {
            ...
            regionStatus: { PENDING: 'في انتظار النتائج', PARTIAL: 'نتائج جزئية', FINAL: 'نتائج نهائية' }, regionPending: 'النتائج مازال ما تعلناتش فهاد الجهة.', regionSeats: '{count} مقعد معلن', regionAllocated: '{count} مقعد مخصص', seats: 'مقاعد', seat: 'مقعد', winner: 'الفائز', constituency: 'الدائرة',
        ...
        fr: {
            ...
            regionStatus: { PENDING: 'En attente', PARTIAL: 'Résultats partiels', FINAL: 'Résultats définitifs' }, regionPending: 'Aucun résultat n’a encore été publié pour cette région.', regionSeats: '{count} sièges déclarés', regionAllocated: '{count} sièges attribués', seats: 'sièges', seat: 'siège', winner: 'Élu·e', constituency: 'Circonscription',
    ...
    function partyRow(party, compact) {
        ...
        const details = element('div', 'election-region-party-copy');
        details.append(element('span', 'election-region-party-name', party.name));
        if (party.winners?.length) {
            ...
            details.append(winners);
        }
        row.append(logo(party), details, element('strong', 'election-region-party-seats', `${number(party.totalSeats)} ${party.totalSeats === 1 ? copy.seat : copy.seats}`));
        return row;
    }

    function winnerRow(winner) {
        const row = element('div', 'election-region-winner');
        const person = element('div', 'election-region-winner-field election-region-winner-person');
        person.append(
            element('span', 'election-region-winner-label', copy.winner),
            element('span', 'election-region-winner-name', winner.candidateName)
        );
        const place = element('div', 'election-region-winner-field election-region-winner-place');
        place.append(
            element('span', 'election-region-winner-label', copy.constituency),
            element('span', 'election-region-winner-constituency', winner.constituencyName)
        );
        row.append(
            person,
            place
        );
        if (winner.votes != null) row.append(element('span', 'election-region-winner-votes', format(copy.votes, { count: number(winner.votes) })));
        return row;
    }
